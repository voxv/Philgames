YUI.add("homehead", function(Y) {
	...
	Y.HomeHeadView = Y.Base.create('homeheadview',Y.View,[],
	{
		...
		render:function()
		{
			...
			var hostscore_score = this.hostscore_score = Y.Node.create('<div class="homehead_oneplayerscore">'+game.players.self.score+'</div>');
			...
			for (var i in players_info)
			{
				if (players_info.hasOwnProperty(i))
				{
					...
					var otherscore_score = this.otherscore_score = Y.Node.create('<div class="homehead_oneplayerscore">'+game.players.others[i].score+'</div>');
