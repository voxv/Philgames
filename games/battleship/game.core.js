...
game_core.prototype.client_onstartrealgame = function(data) {

	thisapp.get('activeView').startGame(data);
};

game_core.prototype.client_onwaitforother = function()
{
	thisapp.get('activeView').sayWide('Un moment, '+otherplayername+' n\'a pas fini de placer ses bateaux.')
			...
			switch(command) {
				case 's': //server message
					...
					switch(subcommand) {
						...
						case 'srg':  // start real game' :
							var vv = JSON.parse(commanddata)
							//alert('commanddata:'+vv)
							//alert('commanddata[]'+vv['host']);
							 parr.client_onstartrealgame(vv);  break;
						case 'wo':  // wait for other to place ships
							 parr.client_onwaitforother(commanddata);  break;
