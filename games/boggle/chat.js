YUI.add("chat", function(Y) {
	...
	Y.ChatView = Y.Base.create('chatview',Y.View,[],
	{
		initializer: function()
		{
			...
			var panel = Y.Node.create('<div class="chatpanel_back"></div><div  class="chatinput"><input type="text" size=80 id="chatinput" maxlength=45/></div>');
	...
	Y.ChatViewMini = Y.Base.create('chatviewmini',Y.View,[],
	{
		initializer: function()
		{
			var container = this.get('container');
			container.set('className',"chatpanelmini_back_tot");
			console.log('chatviewmini!')
			var panel = Y.Node.create('<div class="chatpanelmini_back"></div><div  class="chatinputmini"><input type="text" size=79 id="chatinputmini" maxlength=45/></div>');
			container.append(panel);
			chatentrylist = this.chatentrylist = new Y.ModelList();
			chatentrylist.after('add',this.addChatEntry,this);
		},
    	events:
    	{
			'#chatinputmini': {keypress: 'sendMessage'},
		},
		render: function()
		{
			setTimeout(" if (thisapp.get('activeView').get('container').one('#chatinput')!=undefined) { thisapp.get('activeView').get('container').one('#chatinput').focus(); }",1000);
		...
		addChatEntry: function(e)
		{
			...
			var newv = new Y.ChatEntryModelView({model:this.list.item(list_size)});
			newv.render();
			this.get('container').one('.chatpanelmini_back').append(newv.get('container'));
			this.get('container').one('#chatinputmini').set('value','');
			this.get('container').one('.chatpanelmini_back').set('scrollTop',this.get('container').one('.chatpanelmini_back').get('scrollHeight'));
		}
	});
